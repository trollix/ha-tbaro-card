  ...
  */
  ...
  `;
    ...
    const pressureLabels = labelPressures.map((pressureValue: number) => {
      ...
      const labelPoint = this.polar(cx, cy, labelRadius, labelAngle); 
  ...
  const svgIcon = show_weather_icon
    ...
    //----------------------------------------
    const pressureValueY = isHalfGauge ? 131 : cy + 74;             // 156
    const pressureUnitY = isHalfGauge ? 145 : pressureValueY + 17;  // 171
